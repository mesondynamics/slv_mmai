...
require_pattern NonSecure/App/Src/ecu_network.c '#define CONTROL_MAX_SENDERS[[:space:]]+6U' 'Legacy six-sender control capacity is retained'
require_pattern NonSecure/App/Src/ecu_network.c 'datagram[.]control[.]emergency_stop_on != 0U' 'Legacy emergency field is promoted to fail-safe emergency authority'
require_pattern NonSecure/App/Src/ecu_data_model.c 'run_permit_on = [(]control->emergency_stop_on == 0U[)]' 'Legacy E-stop semantic is inverted at the physical K12 boundary'
require_pattern NonSecure/App/Src/ecu_network.c 'udp_bind[(]transmit_pcb, IP_ANY_TYPE, ECU_STATUS_PORT[)]' 'Legacy status source port remains UDP/50001'
...
require_pattern NonSecure/App/Src/ecu_protocol.c '_Static_assert[(]sizeof[(]ECU_LegacyStatusPayload[)]' 'Legacy UDP status ABI has a compile-time size guard'

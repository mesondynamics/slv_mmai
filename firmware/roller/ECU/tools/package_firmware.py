...
def main() -> int:
    ...
    if version[3]:
        version_text += f"+{version[3]}"
    dependency_version = f"{version[0]}.{version[1]}.{version[2]}"
    output_dir = args.output_dir or PROJECT / "artifacts/firmware" / version_text
    ...
    with tempfile.TemporaryDirectory(prefix="ecu-package-") as temp_name:
        ...
        package = temp / f"roller-ecu-{version_text}.recu"

...
| 控制 | UDP 50002，普通命令仅接受 `172.16.0.10`，推荐 20 Hz |
...
非紧急 V1/V2 控制、全部调参/遥测订阅及 OTA 都只接受固定台架端
`172.16.0.10`；同一 `sender_id` 的活动会话还绑定首次接受的源地址，其他源不能
续租、RELEASE 或清故障。结构和校验完整的网络急停仍在源地址、sequence 和其他
...
ECU 授权不绑定 UDP 源端口，因此整台 `172.16.0.10` 主机属于同一个信任域；该
主机上的其他进程可用更新的 sequence 接管相同 sender。调试 UI 对转向状态额外
...
远程 1 > IDLE。最多保存 6 个发送方；租约年龄 250 ms 仍有效，251 ms 起失效并
...
watchdog 之前恢复非零输出。普通控制还必须来自 `172.16.0.10`，活动发送方按源地址绑定。无有效控制源
...
CAN2 转向速率。转向必须先收到新鲜的 `STEERING_STATUS`，再人工确认使能；

...
不同车辆接收油门、速度及前进/后退指令的机构不同。下表汇集相应的公开改装路径，供其他车型选择与溯源；**本项目针对所用车辆选择的是在低压先导油路增设比例阀**，本 ECU 设计了与之对应的两路阀线圈驱动和电流采样。其余路径仅为公开案例，不表示本 ECU 已实现机械油门、速度电位器、电子节气门或电驱控制器的行驶控制接口。

| 原车机构 | 改装路径 | 在本项目中的定位 | 公开来源 |
| --- | --- | --- | --- |
| 具有低压先导油路的液压行驶机构 | 在先导油路增设比例阀，通过阀线圈电流调节先导压力和原车主阀；具体行驶功能由原车液压回路决定 | **本项目选用**；ECU 提供两路比例阀驱动与电流采样 | [2003 年 CASE CX160 遥控改装](#s60)、[AgOpenGPS 液压硬件](#s48)、[OpenHumidistat 恒流驱动](#s49)、[ADI CN0415](#s38) |
| 机械油门或速度操纵杆 | 用执行器推动原控制件，行程与位置反馈按原车机构适配 | 其他车型的公开参考；本 ECU 未实现 | [ROS 2 Autonomous Tractor](#s59) 展示直线执行器控制加速 |
| 原车速度电位器 | 对可接入的速度设定信号，参考模拟速度指令接入方式 | 其他车型的公开参考；本 ECU 未实现 | [OpenPodcar](#s43) 展示原车速度电位器的电控接入 |
| 电子节气门或电驱控制器 | 在车型和控制器支持的条件下，使用其可验证的电子控制接口；具体接口依车型而定 | 其他车型的公开参考；本 ECU 未实现 | [rusEFI](#s54) 展示电子节气门链路；[OVCS](#s55) 展示电驱车辆多部件 CAN 集成 |
...
速度反馈与速度命令是不同的接口。公开方案中，车速可来自可解析的原车 CAN、车速脉冲或独立速度传感器；[RetroPilot Ocelot](#s56) 展示了原始 VSS 输入与已有 CAN 信息的转换。本项目车辆最终采用的车速反馈及前进/后退信号接法，仍需依据原车接口确定；上述来源不表示所有反馈路径均已在本 ECU 上完成接入。
...
本目录 ECU 接收外部命令，通过通信接口与外部执行装置或原车控制系统连接，通过自身输出连接继电器及针对低压先导油路改装的两路比例阀，并采集可接入的模拟量、数字量和脉冲量。机械油门、速度电位器、电子节气门及电驱控制器在上文仅作为其他车型的公开路径，并非本 ECU 已实现的行驶控制功能。外部转向电机及其功率控制器也不包含在本 ECU 原理图中。原理图为 `PROJECT1.kicad_sch`、`PAGE1.kicad_sch`～`PAGE9.kicad_sch`；下文从具体电路追溯到器件资料、参考设计和开源项目。

### 比例阀驱动的公开电路来源

<a id="sol-os-01"></a>

本 ECU 的两路比例阀驱动主要面向**在车辆低压先导油路增设电液比例阀**的改装方式：PAGE7 为两个阀线圈提供低边驱动及电流采样硬件，通过调节先导压力影响原车主阀。此设计参考 [2003 年 CASE CX160 案例](#s60) 的液压接入思路；该案例使用 12 个阀及独立阀控制器，本 ECU 的两路硬件并非整套挖掘机液压控制系统。两路阀的用途、行驶方向映射及所需电流范围，仍由目标车辆的先导油路和主阀决定。

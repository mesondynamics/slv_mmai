...
不同车辆接收油门、速度及前进/后退指令的机构不同。下表汇集相应的公开改装路径，供其他车型选择与溯源；**本项目针对所用车辆选择的是在低压先导油路增设比例阀**，本 ECU 设计了与之对应的两路阀线圈驱动和电流采样。其余路径仅为公开案例。

| 原车机构 | 改装路径  | 公开来源 |
| --- | --- | --- |
| 具有低压先导油路的液压行驶机构 | 在先导油路增设比例阀，通过阀线圈电流调节先导压力和原车主阀；具体行驶功能由原车液压回路决定 | [2003 年 CASE CX160 遥控改装](#s60)、[AgOpenGPS 液压硬件](#s48)、[OpenHumidistat 恒流驱动](#s49)、[ADI CN0415](#s38) |
...
| 电子节气门或电驱控制器 | 在车型和控制器支持的条件下，使用其可验证的电子控制接口；具体接口依车型而定 | [rusEFI](#s54) 展示电子节气门链路；[OVCS](#s55) 展示电驱车辆多部件 CAN 集成 |
...
ECU 接收外部命令，通过通信接口与外部执行装置或原车控制系统连接，通过自身输出连接点击、继电器及针对低压先导油路改装的两路比例阀，并采集可接入的模拟量、数字量和脉冲量。
...
[ADI CN0415](#s38) 是比例电磁阀驱动的主要公开参考：其说明了 PWM、线圈电流监测、闭环控制和 dither。CN0415 采用低边开关与高边分流采样；本项目借鉴功能链路，在 PAGE7 使用低边分流采样。[S21](#s21)、[S39](#s39)、[S40](#s40)、[S41](#s41) 和 [S42](#s42) 补充电流检测与汽车比例阀资料。
...
本文区分器件厂商资料、开源实现和公开商业产品。来源编号用于追溯。以下等级仅用于 ECU 电路的来源映射。
